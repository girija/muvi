class TamilMovieStar < ActiveRecord::Base
  require 'open-uri'
  require 'nokogiri'

  set_table_name 'tamil_movie_star_data'
 
  def self.add_cast(tamil_movie_id, movie_id)
    cnt = 0
    movie_star = TamilMovieStar.where(:movie_id => tamil_movie_id).order('id desc')
