function show_tweet_comments(id){
  $("#tweet_reply_"+id).toggle();
  $('#tweet_container').masonry({
    itemSelector : '.mas_celeb_tweet_item'
  });
}
function show_tweet_comments_ontext(id){
  $("#tweet_reply_"+id).show();
}

function show_review_comments(id){
  $("#review_reply_"+id).toggle();
...
function show_comments_ontext(id){
  $("#review_reply_"+id).show();
  $('#mas_container').masonry({
    itemSelector : '.mas_item'
  });
...
function show_user_review_comments(id){
  $("#user_review_reply_"+id).toggle();
  $('#mas_container_u_reviews').masonry({
    itemSelector : '.mas_item'
  });
}
function show_user_comments_ontext(id){
  $("#user_review_reply_"+id).show();
  $('#mas_container_u_reviews').masonry({
    itemSelector : '.mas_item'
  });
...
function check_event_submit_comment(evt,id){
...
$(document).ready(function($){
    ...
    $('input:text[placeholder]').placeholderLabel();
     ...
     _renderMenu: function( ul, items ) {
        ...
        $(".ui-autocomplete1").css('z-index', 10000);
	$.each( items, function( index, item ) {
            if ( item.cat != currentCategory ) {
                ul.append( "<li class='autocomplete-header'>" + item.cat + "</li>" );
                currentCategory = item.cat;
            }
            if(index+1 == items.length){
                $( "<li></li>" )
                .data( "item.autocomplete", item )
                .append( "<a>" + "<span style='margin-top:5px;position:absolute;padding-left:10px;font-size:122x;'>"+ item.label +"</span></a>" )
                .appendTo( ul )
                ul.append("<div style='font-size:12px;float:right;font-weight:normal'><a href='javascript:void(0);' onclick='document.forms[0].submit();'>see all search suggestions</a></div>" );
            }else{
                $( "<li></li>" )
                .data( "item.autocomplete", item )
                .append( "<a>" + "<span style='margin-top:5px;position:absolute;padding-left:10px;font-size:122x;'>"+ item.label +"</span></a>" )
    ...
    source: function(request, response) {
	    ...
	    var cssattr = {'width':'347px','left':'436px','margin-top':'12px','position':'fixed','top':'25px'};
            $(".ui-autocomplete1").css(cssattr);
            return;
        }
    ...
    select: function( event, ui ) {
                if(ui.item.cat == "Users"){
                    window.location.href = "/profile/"+ui.item.id+"/"+ui.item.label;
                }else if(ui.item.cat == "Stars"){
                    window.location.href = "/stars/"+ui.item.permalink;
                }else{
                    window.location.href = "/movies/"+ui.item.permalink;
                }
    }
    });
    ...
    $(".items").sortable();
    $("#celebs  input[id^=item_]").live("keydown.autocomplete", function() {
        $(this).autocomplete({
    ...
    $("#movies  input[id^=item_]").live("keydown.autocomplete", function() {
        $(this).autocomplete({
    ...
    $(".removeBox").live("click",function(){
        ...
        if (!$('form .row').length) {
          appendMore();
        } else if($("input[id^=item_]:last").val() !== ''){
          appendMore();
        }
    });
...
function appendMore() {
    if($('form .row').length < 10){
     if($("input[id^=item_]:last").val() !== ''){
     for (var i=1; i <= 10; i++)
     {
       if ($('#item_'+i).length == 0){
           var box_no = i;
       ...
       var box = $('<div class="row"><div class="name left"><input type="text" value="" style="width:339px" placeholder="Enter the name of movie or celebrity" name="item[]" id="item_'+ box_no +'" class="ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true"><input type="hidden" value="0" name="id_item[]" id="id_item_'+ box_no +'"><div class="clear"></div><textarea style="height:20px;margin-left:0px" rows="1" placeholder="Add a note saying why do you like" name="note[]" id="note_'+ box_no +'" cols="40"></textarea></div><div style="margin-left:5px;margin-top:1px;" id="img_item_'+ box_no +'" class="image left"><img height="45" width="45" src="/images/no-image.png" alt="No-image"></div><a href="#" class="removeBox"><img style="float:right" src="/images/cross.png" alt="Cross"></a><div class="clear"></div></div>');
        $('.items').append(box);
    }
   }
}
...
function critics_reviews_sort_new(movie_id,value){
  var url='/critics_reviews?id='+movie_id+'&block=inside&sort='+value;
  $.getScript(url);
  return false;
}
...
function open_window(){
  window.open("/auth/facebook", "", "width=650,height=500");
}
...
function displayFancyBox(url, form_id){
 $.fancybox({
...
var submitFans = false;
var showPoints = false;
var showCelebPoints = false;
var allVals = [];
function submitForm(form_id, points, check_celeb){
 if(check_celeb == true) {
   if ($("input:checkbox[name=celebrity[]]:checked").length  == 0) {
     alert("Please select a celebrity");
     return false;
   }
   showCelebPoints = true;

   $("input:checkbox[name=celebrity[]]:checked").each(function(){
      allVals.push($(this).val());
   });
   $("#selected_celeb_id").val(allVals);
 }
 $.fancybox.close();
 submitFans = true;
 showPoints = points;
...
function badgePoints(message, points){
   var notice = '<div class="yellow_purr">'
                 + '<p>Congratulations! You got <b> +'+ points + '</b> points for ' + message +'</p>'
                 + '</div>';
    $( notice ).purr({
      usingTransparentPNG: true
    });
  $("#purr-container").removeAttr("style");
  $("#purr-container").css("z-index", 1200);
  $("#purr-container").css("top", 0);
  $("#purr-container").css("left",($(document).width()-$(".yellow_purr").width())/2);
  return false

   var notice = '<div class="notice">'
                 + '<div class="notice-body">'
                 + '<h3>Congratulations!</h3>'
                 + '<p>You got <b> +'+ points + '</b> points for ' + message +'</p>'
                 + '</div>'
...
$(document).ready(function($){
 $("#_not_required_new_review").live("ajax:beforeSend", function(){
  ...
  if (submitFans == false){
    if(review_rating >= 3) {
      var movie_url = '/badges/movie/'+ $('#want_to_see_movie_id').val();
      displayFancyBox(movie_url ,'new_review');
      return false;
    }
  }

    //$("#new_review").attr('data-remote',true);
    badgePoints('rating', 5);
    if (showCelebPoints == true)
    {
      for (i=0;i<allVals.length;i++)
      {
        badgePoints('becoming a fan', 5);
      };
    }
    return true;
  }
...
$("#_not_requried_fan_form[action = '/fans']").live("ajax:beforeSend", function(evt, xhr, settings){
  if(submitFans == false) {
  var celeb_url = '/badges/celebrity/'+ $('#fans_celebrity_id').val();
  var fancy = displayFancyBox(celeb_url ,'fan_form');
  return false;
  }
