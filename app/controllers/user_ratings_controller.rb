class UserReviewsController < ApplicationController
  ...
  def index
    ...
    #reviews_query = "((select 'movie_review' as model,user_id,description,rating,'' as review,'' as twitter_name,created_at from reviews where reviews.movie_id=#{@movie.id} ) UNION (select 'movie_tweet' as model,user_id,content,rating,review,twitter_screen_name,tweeted_on from tweets WHERE ( tweets.interest = 'f') AND (tweets.review = 'pos' or tweets.review = 'neg' ) AND (tweets.movie_id = #{@movie.id}) ) order by twitter_name asc) as temp"
     reviews_query = "((select 'movie_review' as model,user_id,description,rating,'' as review,'' as twitter_name,created_at from reviews where reviews.movie_id=#{@movie.id} ) ) as temp"
     twitter_query = "((select 'movie_tweet' as model,user_id,content,rating,review,twitter_screen_name as twitter_name,tweeted_on from tweets WHERE ( tweets.interest = 'f') AND (tweets.review = 'pos' or tweets.review = 'neg' ) AND (tweets.movie_id = #{@movie.id}) ) order by twitter_name asc) as temp"
    ...
    @movie_user_reviews = result_review.paginate(:page => params[:page], :per_page =>  4)
    @movie_tweets =  result_twitter.paginate(:page => params[:page], :per_page =>  4)
